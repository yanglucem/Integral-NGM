...
# set true parameters and initial condition, note, x0 = y0
y0 = 0.4
...
T = 4
h = 0.04
noiseset = np.array([10, 15, 20, 25])

# svaing parameters
mape_pars = np.zeros((7, len(noiseset)))
mean_pars = np.zeros((7, len(noiseset)))
stds_pars = np.zeros((7, len(noiseset)))
...
''' main loop'''
for iter_noise in range(len(noiseset)):
    
    noiselevel = noiseset[iter_noise]
    
    # generate noise free data
    t_train = np.linspace(0, T, int(T / h) + 1)
    datalen = len(t_train)
    num_test = 3
    ...
    x = x_y[0:datalen, 0]
    y = x_y[0:datalen, 1]
    y_grey = odeint(verhulst_grey, y0, t_train, args=(par_y,), 
                    rtol=1e-12, atol=1e-12)
    
    # compare numarical soluation to Cumsum series
    y_cum = np.append(x[0], x[0] + np.cumsum(h * x[1:len(x)]))
    e_cum = abs((y - y_cum) / y) * 100
    solution = np.vstack((x, y, y_cum, e_cum))
    # np.savetxt("output/solution.csv", solution, delimiter=',')
    
    # plt.figure(1)        
    # plt.plot(t_train, y, color = 'b')
    # plt.plot(t_train, x, color = 'k')
    ...
    for iter_nrep in range(nrep):
        ...
        pars_im[:, iter_nrep] = np.insert(np.append(par_im, x0_im), [0],
                                          noiselevel, axis = 0)
        ...
        y_grey = odeint(verhulst_grey, xn[0], tspan, args=(par_grey,),
                        rtol=1e-12, atol=1e-12)
        tlen = len(y_grey)
        x_grey = np.append(y_grey[0], (y_grey[1:tlen] 
                                       - y_grey[0:tlen-1]) / h)
        ape_par_grey = abs( (par_y - par_grey) / par_y ) * 100 
        mse_xgrey = (x_grey - x_y[:,0]) ** 2 
        
        pars_grey[:,iter_nrep] = np.insert(par_grey, [0],
                                           noiselevel, axis = 0)
    ...
    mape_pars[:, iter_noise] = np.append(ape_pars_grey.mean(axis = 1), 
                                                   ape_pars_im.mean(axis = 1))
    mean_pars[:, iter_noise] = np.append(pars_grey.mean(axis = 1),
                                                   pars_im.mean(axis = 1))  
    stds_pars[:, iter_noise] = np.append(pars_grey.std(axis = 1), 
                                                   pars_im.std(axis = 1))
    
    
    '''fitting and forecasting error boxplot'''
    table_xim_train = np.concatenate((table_xim_train, 
                                      rmse_xim[0, :].T.reshape(nrep, 1)),  
                                     axis = 1)
    table_xim_test = np.concatenate((table_xim_test, 
                                     rmse_xim[1, :].T.reshape(nrep, 1)),  
                                    axis = 1)
    table_xgrey_train = np.concatenate((table_xgrey_train, 
                                        rmse_xgrey[0, :].T.reshape(nrep, 1)),  
                                       axis = 1)
    table_xgrey_test = np.concatenate((table_xgrey_test, 
                                       rmse_xgrey[1, :].T.reshape(nrep, 1)),
                                      axis = 1)
    
    
    '''save data for box plot'''
    np.savetxt("Simu1_output/datalen_xim_train.csv", 
               table_xim_train, delimiter=',')    
    np.savetxt("Simu1_output/datalen_xim_test.csv", 
               table_xim_test, delimiter=',')
    np.savetxt("Simu1_output/datalen_xgrey_train.csv", 
               table_xgrey_train, delimiter=',')
    np.savetxt("Simu1_output/datalen_xgrey_test.csv", 
               table_xgrey_test, delimiter=',')
    ...
    np.savetxt("Simu1_output/datalen_pars_im.csv", table_pars_im, delimiter=',')    
    np.savetxt("Simu1_output/datalen_pars_test.csv", table_pars_grey, delimiter=',')
