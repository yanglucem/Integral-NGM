# -*- coding: utf-8 -*-
"""
Created on Mon Aug 30 13:46:37 2021
...
Simulation 2: grey Lokta-Velterra model under fixed data length
"""
...
num_test = 3
Tspan = T + num_test * h
noiseset = np.array([4, 8, 12, 16])

# saving parameters for table 1

mape_pars = np.zeros((12, len(noiseset)))
mean_pars = np.zeros((12, len(noiseset)))
stds_pars = np.zeros((12, len(noiseset)))
...
for iter_noise in range(len(noiseset)):
    ...
    for iter_nrep in range(nrep):
        ...
        x_im_sol = solve_ivp(lotka_int, [0, Tspan], np.append(x0_im, y0_im), 
                         args=(par_im,), dense_output=True, method='DOP853')
        ...
        ''' note that here we use the true value of x0 to solve the differential equation'''
        y_grey_sol = solve_ivp(lotka_grey, [0, Tspan],  x0, 
                         args=(par_grey,), dense_output=True, method='BDF')
        ...
        pars_grey[:,iter_nrep] = np.insert(par_grey, [0], noiselevel, axis = 0)
    ...
    '''data for boxplot'''
    np.savetxt("Simu2_output/datalen_x1im_train.csv", table_x1im_train, delimiter=',')    
    np.savetxt("Simu2_output/datalen_x1im_test.csv", table_x1im_test, delimiter=',')
    np.savetxt("Simu2_output/datalen_x1grey_train.csv", table_x1grey_train, delimiter=',')
    np.savetxt("Simu2_output/datalen_x1grey_test.csv", table_x1grey_test, delimiter=',')
    
    np.savetxt("Simu2_output/datalen_x2im_train.csv", table_x2im_train, delimiter=',')    
    np.savetxt("Simu2_output/datalen_x2im_test.csv", table_x2im_test, delimiter=',')
    np.savetxt("Simu2_output/datalenx2grey_train.csv", table_x2grey_train, delimiter=',')
    np.savetxt("Simu2_output/datalen_x2grey_test.csv", table_x2grey_test, delimiter=',')
